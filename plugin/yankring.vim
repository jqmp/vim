...
function! s:YRMapsCreate()
    ...
    if s:yr_has_voperator == 1
        ...
        " Loop through and prompt the user for all buffer connection parameters.
        for key in o_maps
            " changed this code to respect previous omaps
            " hopefully haven't broken anything
            " -- JK
            if maparg(key, "o") != ""
                exec 'omap <expr>' key 'YRMapsExpression("<SID>","'. escape(maparg(key, 'o'),'\"'). '")'
            else
        ...
        for key in split(g:yankring_zap_keys)
            if maparg(key, "o") != ""
                exec 'omap <expr>' key 'YRMapsExpression("<SID>","'. maparg(key, 'o'). '", 1)'
            else
